...
// Assicurati che 'fs' sia importato correttamente per 'unlinkSync' e 'existsSync'
import {readdirSync, statSync, unlinkSync, existsSync, readFileSync, rmSync, watch} from 'fs'; 
import * as fs from 'fs'; // <-- AGGIUNTA/VERIFICA IMPORTAZIONE per usare fs.existsSync
...
if (reason == 405) {
    // CORREZIONE PER ENOENT: Verifica se il file creds.json esiste prima di eliminarlo
    const credsPath = path.join(global.authFile, 'creds.json');
    if (fs.existsSync(credsPath)) {
        fs.unlinkSync(credsPath); // Eliminazione sicura
        console.log(chalk.bold.redBright(`[ ⚠️ ] 𝐂𝐨𝐧𝐧𝐞𝐬𝐬𝐢𝐨𝐧𝐞 𝐬𝐨𝐬𝐭𝐢𝐭𝐮𝐢𝐭𝐚, 𝐫𝐢𝐚𝐯𝐯𝐢𝐨 𝐢𝐧 𝐜𝐨𝐫𝐬𝐨...\n𝐒𝐞 𝐚𝐩𝐩𝐚𝐫𝐞 𝐮𝐧 𝐞𝐫𝐫𝐨𝐫𝐞, 𝐫𝐢𝐜𝐨𝐦𝐢𝐧𝐜𝐢𝐚 𝐜𝐨𝐧: 𝐧𝐩𝐦 𝐬𝐭𝐚𝐫𝐭`)); 
        process.send('reset');
    } else {
        console.log(chalk.bold.yellow(`[ ⚠️ ] 𝐂𝐨𝐧𝐧𝐞𝐬𝐬𝐢𝐨𝐧𝐞 𝐬𝐨𝐬𝐭𝐢𝐭𝐮𝐢𝐭𝐚, riavvio in corso (il file creds.json non è stato trovato o era già stato rimosso).`));
        process.send('reset');
    }
}
if (connection === 'close') {
